...
__export(index_exports, {
  ...
  addAmbientAudio: () => addAmbientAudio,
...
async function generateAmbientAudio(sceneType, outputPath, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const ffmpeg = (0, import_child_process.spawn)("ffmpeg", args);
    let stderr = "";
...
async function getVideoDuration(videoPath) {
  return new Promise((resolve, reject) => {
    const ffprobe = (0, import_child_process.spawn)("ffprobe", [
      "-v",
      "error",
      "-show_entries",
      "format=duration",
...
async function addAmbientAudio(videoPath, sceneType, outputPath, options) {
  ...
  const tempAudioPath = (0, import_path.join)("/tmp", `ambient_${Date.now()}.mp3`);
  try {
    ...
    try {
      await import_fs.promises.unlink(tempAudioPath);
    } catch {
    }
  }
}
